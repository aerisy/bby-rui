...
+function ($) {
  ...
  Modal.prototype.initOptions = function () {
    ...
    if (this.options.remote) {
      $ele
        .find('.modal-content')
        .load(this.options.remote + "&sui_rcode=" + Math.random(1000), $.proxy(function () {
          //YBB edit
          //表单校验初始化
...
!function ($) {
 ...
 /* BUTTON PUBLIC CLASS DEFINITION
  * ============================== */
  //YBB 修改sui3中的toast，因为和toastr.js冲突，修改CALSS名toast->toast-sui，同时css中也做修改
  var template = '<div class=" toast-sui"><h5 class="toast_text msg-con"></h5><s class="msg-icon"></s></div>';
  ...
  Toast.prototype = {
    Constructor : Toast,
     render: function(){
      ...
      var message = ".toast-sui";
      var text = ".toast_text";
      var cssPrefix = "toast-sui-";
      $(message).remove();
      this.el = $(template);
    ...
    hide : function(callback){  // hide : function(duration, callback);
      this.el.removeClass('toast-sui-in');
      var self = this.el;
